...
import { Box, Tabs,Tab,Typography, Checkbox, Button, IconButton, RadioGroup,FormControlLabel, Radio,Chip,Stack,Paper, Grid2 as Grid } from "@mui/material";
...
const Component =  (props) => {
    ...
    useEffect(() => {
        ...
        initialDbData.forEach((row) => {
            ...
            next[dateKey] = {
                ...
                AvailabilityDoctor: row.AvailabilityDoctor ?? 1,
                Windows: windows,
                checked: true,
            };
        });
    ...
    const visibleWeeks = availabilityType === "Weekly" ? weeks.slice(0, 1) : weeks; // only Week 1 for Weekly
    ...
    const openAddWindow = (date) => {
      const key = dayjs(date).format("YYYY-MM-DD");
      setEditing({ open: true, dateKey: key, windowIndex: null, start: dayjs(date).hour(10).minute(0), end: dayjs(date).hour(12).minute(0) });
    };
    ...
    const commitEdit = () => {
      ...
      const data = {
          ...
          Type: newDate.Type,
          AvailabilityDoctor: newDate.AvailabilityDoctor,
          Slots: slots
      };

      if(newDate.id) data['AvailabiltyId'] = newDate.id;
      if(onSave) onSave(data);
      setEditing({ open: false, dateKey: null, windowIndex: null, start: null, end: null });
    };
      ...
      <LocalizationProvider dateAdapter={AdapterDayjs}>
        <Box sx={{ p: 2 }}>
          <Stack direction="column" spacing={5} alignItems="flex-start">
            ...
            {/* Session Span */}
            <Box sx={{ width: '90%' }}>
              <Typography variant="body2" sx={{ fontWeight: 'bold',  mb: 3 }}>Session span:</Typography>
              <RadioGroup row value={String(sessionSpan)} onChange={(e) => setSessionSpan(parseInt(e.target.value))}
                  sx={{ borderRadius: 2, border: "1px solid #E7E7E7", padding: '24px 40px' }}
              >
                  <Grid container rowSpacing={1} justifyContent='space-between' columnSpacing={{ xs: 1, sm: 2, md: 3 }} sx={{ width: '100%'}}>
                      <Grid size={5}>
          ...
          {/* Month Picker (drives which month's days render) */}
          ...
          {availabilityType === 'SpecificDate' && (
              <Tabs value={tabIndex} onChange={(e, v) => setTabIndex(v)}>
                  {(availabilityType === "Weekly" ? getMonthWeeks(selectedMonth).slice(0, 1) : getMonthWeeks(selectedMonth)).map((_, i) => (
          ...
          <Box sx={{ mt: 2 }}>
            {/* Days for the active (visible) week */}
            {(availabilityType === "Weekly" ? getMonthWeeks(selectedMonth).slice(0, 1) : getMonthWeeks(selectedMonth))[tabIndex]?.map((date) => (
        ...
        {editing.open && (
          <Box
            ...
            onClick={() => setEditing({ open: false, dateKey: null, windowIndex: null, start: null, end: null })}
          >
            <Box
            ...
            >
                ...
                <Stack spacing={2}>
                    ...
                    <Stack direction="row" spacing={1} alignItems="center">
                      <Chip label={`Sessions: ${generateSlotsFromWindow(editing.start, editing.end, sessionSpan, () => 0).length}`} />
                      <Typography variant="body2" color="text.secondary">(auto from span {sessionSpan}m)</Typography>
                    </Stack>
