...
import AvailabilityJsonConfig from "config/availability_product_onetomany_oneToMany_config.json";
...
const MapItems = [
    ...
    { navpropname: "", uicomponent: "availability", expand: "Slots", exclude: [], func: Support.AddOrUpdateAvailability }
,    { navpropname: "Slots", uicomponent: "slotslots", expand: "Slots", exclude: [], parent: "availability", child: true }
];

const FetchAvailabilityInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(AvailabilityJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of AvailabilityJsonConfig[elm]) {
...
const FetchAvailabilityDetails = async (availabilityId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (availabilityId) {
            global.Busy(true);
                    ...
                    let rslt = await Api.GetAvailabilitySlotsJoin(availabilityId);
            if (rslt.status) availabilitySlots = rslt.values;
            ...
            rslt = await Api.GetAvailabilitySingle(availabilityId, `$expand=${$expand}`);

            if (rslt.status) {

                const availability = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? availability : availability[source];
                    if (parent && child) {
                        let _tmpItems = [];
            ...
            global.Busy(false);
                ...
                return resolve({ row: item, backRow: backItem, mapitems: availabilitySlots });
    });
}
...
const Extract = async (availabilityId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchAvailabilityInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(availabilityId)) {
                    await FetchAvailabilityDetails(availabilityId, enums).then(({ row, backRow, mapitems }) => {
