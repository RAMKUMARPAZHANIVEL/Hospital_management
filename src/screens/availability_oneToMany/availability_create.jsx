...
import AvailabilityScheduler from "./child/AvailabilityScheduler";
...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, availabilityId, numfields;
        ...
        inlineObjs.forEach(x => {
            ...
            let valueList = [];
            _values.forEach(m => {
                ['action', 'SlotId', 'id'].forEach(z => delete m[z]);
            ...
            availability.push({ key: x.property, value: valueList, type: "collections" });
        });
    ...
    const fetchAvailabilities = async () => {
        ...
        doctorId  = 1;
        // row['Doctor'].find((x) => x.key === 'DoctorId').value;
        expands = "Slots"
    ...
    if (initialized) { setInitialized(false); fetchData(); fetchAvailabilities(); }
                    ...
                    <>
                        ...
                        <Box sx={{ width: '100%' }}>
                            <Grid container sx={{ flex: 1, alignItems: "center", justifyContent: 'flex-start', gap: 1, pt: 1, pb: 1 }}>
                                ...
                                <Button variant="outlined" onClick={() => NavigateTo("/")}>Cancel</Button>
                            </Grid>
                        </Box>
                    </>
                )} */}
