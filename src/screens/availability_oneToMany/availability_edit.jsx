...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, availabilityId, changes, numfields;
        const mapItems = MapItems;

        // Attach inline objects
        let availability = row['availability'];
        availabilityId = row['availability'].find((x) => x.type === 'keyid').value;
        ...
        inlineObjs.forEach(x => {
            ...
            availability.push({ key: x.property, value: vObj, type: "inline" });
        });
        ...
        changes = TrackChanges('availability');
        if (changes.length > 0) {
            numfields = Helper.GetAllNumberFields(availability);
            if (numfields.length > 0) Helper.UpdateNumberFields(availability, numfields);
            rslt = await Support.AddOrUpdateAvailability(availability, dropDownOptions, []);
            if (rslt.status) {
                availabilityId = rslt.id;
            } else { return; }
        }
        ...
        for (let i = 0; i < updateChild.length; i++) {
            let _data = updateChild[i];
                        let SlotsAvailabilityMapId = null;
            if (_data.Deleted && !Helper.IsNullValue(_data.SlotId)) {
                SlotsAvailabilityMapId = availabilitySlots.find(x => x.SlotId === _data.SlotId && x.AvailabiltyId === availabilityId).Id;
            }
            rslt = await Support.AddOrUpdateAvailabilitySlots(SlotsAvailabilityMapId, availabilityId, _data);
        ...
        for (let i = 0; i < mapItems.length; i++) {
            ...
            if (changes.length > 0) {
                // Add or Update the Availability and navigation entity if it is deos not exist
                let navItem = availability.find(x => x.uicomponent === mapItems[i].uicomponent);
                if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                    ...
                    if (rslt.status) {
                        data = [
                            { key: "AvailabiltyId", value: parseInt(availabilityId) },
                            { key: navItem.key, value: parseInt(rslt.id) }
                        ];
                        rslt = await Support.AddOrUpdateAvailability(data, dropDownOptions);
        ...
        global.AlertPopup("success", "Availability is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
