// MedicationsOneToManyForm.jsx
// A clean, config‑driven one‑to‑many CRUD form for medications (JavaScript version)
// Tech: React 18, MUI v5, react-material-ui-form-validator
...
function MedicationRow({ row, config, mode, onSave, onDelete, onCancelNew }) {
  ...
  const handleSave = () => {
    ...
    if (isNew) {
      ...
      onSave({ mode: "edit", id: row[keyId], payload: { ...patch, MedicationId : row[keyId] } });
    }
    setEditing(false);
  };
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
        <Stack direction="row" spacing={1} alignItems="center">
          <Typography variant="subtitle1" fontWeight={700}>
            {row.MedicationName || "New medication"}
          </Typography>
...
export default function Component({
  ...
  const addNewRow = () => {
    setRows((prev) => [{ __localId: crypto.randomUUID(), ...createEmptyRow(config) }, ...prev]);
  };
  ...
  const handleSaveRow = ({ mode: saveMode, id, payload }) => {
    const actions = {id, action: saveMode};
     if (onTableRowUpdated) {
            onTableRowUpdated({ ...actions, rows, keyIdName: keyId, location: 'medication', data: payload });
            // ClearSettings();
        }
    // if (saveMode === "create") {
    //   onCreate?.(payload);
