...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, patientId, numfields;
        ...
        rslt = await Support.AddOrUpdatePatient(patient, dropDownOptions, ['ProfilePicture'], nestedObj);
        if (rslt.status) {
            patientId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Patient and navigation entity if it is deos not exist
            let navItem = patient.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                // Add ProfilePicture
                prodImages = patient.find((x) => x.key === 'ProfilePicture');
