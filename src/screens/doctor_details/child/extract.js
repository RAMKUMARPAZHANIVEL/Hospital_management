import DoctorJsonConfig from "config/doctor_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "doctor", expand: "DoctorImage", exclude: ['DoctorImage'], func: Support.AddOrUpdateDoctor },
    { navpropname: "", complexPropName : "Address", uicomponent: "Address", expand: "", func: Support.AddOrUpdateDoctor, type: "complex" },
    { navpropname: "Qualifications", uicomponent: "qualification", expand: "Qualifications", exclude: [], parent: "doctor", child: true }
]

const FetchDoctorInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(DoctorJsonConfig);
...
const FetchDoctorDetails = async (doctorId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (doctorId) {
            global.Busy(true);
                    ...
                    let rslt = await Api.GetDoctorQualificationsJoin(doctorId);
            if (rslt.status) doctorSlots = rslt.values;
            ...
            rslt = await Api.GetDoctorSingle(doctorId, `$expand=${$expand}`);
            ...
            if (rslt.status) {
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? doctor : doctor[source];
                    if (parent && child) {
                        let _tmpItems = [];
                    ...
                    if (doctor.DoctorImage) {
                        ...
                        if (item['doctor'].find((x) => x.key === "DoctorImage")) {
            ...
            global.Busy(false);
                ...
                return resolve({ row: item, backRow: backItem, mapitems: doctorSlots });
    });
}
...
const Extract = async (doctorId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchDoctorInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(doctorId)) {
                    await FetchDoctorDetails(doctorId, enums).then(({ row, backRow, mapitems }) => {
